...
function SuperDResolution() {
  ...
  const removebgProcessor = async (image) => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/api/superresolution`, formData, {
        headers: {
  ...
  const handleDownloadA = () => {
    if (ResultImageA) {
      ...
      link.download = `${downloadFilename}_Enhanced_bg_A.jpg`;
  ...
  const handleDownloadB = () => {
    if (ResultImageA) {
      ...
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `${downloadFilename}_Enhanced_bg_B.jpg`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    }
  ...
  return (
    <div>
      ...
      <div className='titlewrapper'>
                    <span>Image </span> Enhancer
      </div>
        {ShowUploader ? <div className='preview-container'>
...
export default SuperDResolution
